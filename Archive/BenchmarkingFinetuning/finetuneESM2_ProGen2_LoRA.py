...
class LoggingTrainer(Trainer):
    ...
    # ← note the added `num_items_in_batch=None` here
    def training_step(self, model, inputs, num_items_in_batch=None):
        # call the parent which will handle forward, backward, gradient accumulation, etc.
        loss = super().training_step(model, inputs, num_items_in_batch)

        # log the per-step training loss
        if self.args.local_process_index == 0 and wandb.run is not None:
            wandb.log({
...
def main() -> None:
    ...
    # Create a Trainer instance with LoggingTrainer to handle evaluation memory
